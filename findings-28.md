...
For arbitrary directed graphs, the boundary-path space is naturally realized as the spectrum of the graph diagonal, so these consistent cylinder values are legitimately boundary-state data rather than merely formal path weights.
...
This resembles a transition kernel, but its role here is entirely static:

[
\boxed{
\pi_h(e\mid v)
\text{ is the conditional splitting of a boundary cylinder,}
}
]

not an added temporal update rule.

It is derived from the graph, the critical path-length scale, and the boundary state.
