...
# 5. Validation against the exhaustive level-seven law
...
Every tested profile mean agreed with the exact census well within one standard deviation, and the sampled covariance differed from the exact covariance by only about
...
where the pairs are mean out/in degree, directed two-path density, and the two wedge-type densities.
...
# 9. Dynamic fixed-point evidence
...
# 10. Finite-size interpretation of the coordinates
