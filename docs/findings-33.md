...
Fix a root $o$, a weight $t>0$, the reachable node set $V_o$, and finite Green
values $G_t(o,x)>0$.
...
If $d(o,v)$ is the shortest path length from $o$ to $v$, prefixing a shortest
path $o\to v$ gives
...
Martin compactification embeds points by their normalized Green kernels and
takes the closure.
...
$\mathcal M_{t,F}$ comprises all limits of observations at only the nodes in
$F$.
...
The map into the inverse limit is injective because profiles agreeing on every
finite subset agree at every node.
...
A basic product neighborhood of $y$ depends on only finitely many coordinates,
say those in $F$. Since
...
that neighborhood contains the finite projection of a normalized Green column,
every basic neighborhood of $y$ intersects $\iota_t(V_o)$, so
...
A limiting coordinate value may be approximated arbitrarily well without being
attained by any actual node. It belongs to
...
If every finite profile image is closed, for example, in certain finite or
discrete situations, the closures can be omitted.
...
Martin equivalence defines the profile quotient by identifying histories
satisfying $h_\xi=h_\eta$; this quotient removes route information.

## Profile completion

The compactification adds limiting profiles that need not come from a regular
history.
...
* even one countable sequence.

Consequently, the closure of regular ray profiles can be smaller than the full
...
The bounded system definition alone does not provide one, especially with
infinite branching or moving-endpoint escape.

Therefore the primary node compactification remains
...
These are continuous observables depending on only finitely many node Green
ratios.

If $F,F'$ are finite, sums and products of an $F$-observable and an
$F'$-observable depend only on
...
It separates points of $\mathcal M_t$: two different profiles differ at some
node $v$, and the coordinate observable at $v$ distinguishes them.
...
Every continuous node-level Martin observable is uniformly approximable by
observables involving only finitely many nodes.

Finite node observations do not recover route history; they recover all
continuous information on the already-compressed node compactification.
...
Its support contains only finitely many histories. For each pair with different
full profiles, choose a node distinguishing them; the union of these nodes is a
finite set $F_0$.

On the finite support of $\mu$, the $F_0$-equivalence classes are exactly the
full Martin-equivalence classes.
...
The argument uses the finite support of $\mu$ to obtain the finite set $F_0$.
...
The annihilators and defect modules correspond as follows:
...
Every reachable node has finite compositional distance from $o$, so agreement
on every $B_n(o)$ is exactly agreement at every reachable node.
...
For a unique-route rooted tree, this is the depth at which the rays first
diverge. Two histories in a diamond followed by a common tail have $s_t=\infty$
because no node Green profile distinguishes them, and every pair in complete
layered mixing likewise has $s_t=\infty$.
...
The last map records, for a zero-total combination in a coarse class, the total
coefficient in each finer subclass.

Its kernel consists exactly of combinations already having zero total inside
every finer class, namely $\mathfrak D_{n+1}$. Hence
...
The number of newly distinguishable profile directions is exactly the increase
in the number of observation classes.

For infinite class sets, cardinal subtraction is not canonical, so the
direct-sum module remains the correct invariant.
...
Two histories have the same full Martin profile exactly when they determine the
same branch through every level.

The observation levels compose as
...
The defect quotient at each level records the new branch splitting at that
level.

# 15. Completed and realized branches

Realized branches come from actual regular histories. Completed branches are
coherent limits in the compact finite-profile spaces
...
Therefore the inverse-limit completion contains realized profile branches and
may also contain generalized coherent branches.

The construction parallels completion of finite paths by boundary points, with
normalized Green observations replacing prefixes.
...
* every profile is determined by a compatible system of finite discrete
  profiles;
* the finite-observable algebra is the locally constant algebra;
* its uniform completion is $C(\mathcal M_t)$.

Inverse limits of finite discrete spaces are exactly the standard profinite
construction.
