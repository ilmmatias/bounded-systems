...
That is not a defect. It says that the collapse to one reachability arrow is completely coherent.
...
That is precisely the augmented simplicial chain complex of the open interval:
...
This is exactly the kind of situation in which a spectral sequence is useful: not merely for its limiting group, but because its intermediate pages identify which structural information is lost at each compression stage.
