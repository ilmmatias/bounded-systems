...
[
(c_\tau)*\tau\longmapsto\sum*\tau c_\tau.
]
...
So we should stop studying each set (\overline F_h) in isolation and study the complete quotient map
...
The new information is not just the cardinality of each fiber, but how those fibers vary with (h).
...
Let (\underline{k}*{Z_t}) and (\underline{k}*{Y_t}) denote constant sheaves.
...
It is not topology inside one fiber. It is topology in the **variation of fibers**.
...
We should therefore replace the proposed “higher homology of the quotient end relation” by three better objects.
...
\bar\pi_{t*}\underline{k}*{Z_t}/\underline{k}*{Y_t}.
