...
Thus orthogonality does not disappear mysteriously. It is explicitly destroyed by the endpoint-summation map, with norm amplification (\sqrt{M_n(u)}).
...
This is only a lower bound for the convergence radius. Alternating cancellation can make the true radius much larger.
