...
def dirichlet_cov(alpha):
    ...
    for i in range(len(alpha)):
        ...
        for j in range(len(alpha)):
            if i == j:
                direct = Fraction(alpha[i] * (total - alpha[i]), total * total * (total + 1))
            else:
                direct = Fraction(-alpha[i] * alpha[j], total * total * (total + 1))
            formula = ((mean[i] if i == j else 0) - mean[i] * mean[j]) / (total + 1)
...
def main():
    ...
    for n in (50, 100, 200, 400):
        ...
        mean, var = beta_variance(a + e, b + m - e)
        target = w * (1 - w) / c
    ...
    print(f"verified {checks} exact information-rate identities and convergence checks")
