...
def hom_density(
    ...
    for mapping in product(range(target_vertices), repeat=pattern_vertices):
        if all(
            (mapping[source], mapping[target]) in edge_set
...
def disjoint_union(
    left_vertices: int,
    left_edges: Sequence[Edge],
    right_vertices: int,
    right_edges: Sequence[Edge],
) -> tuple[int, tuple[Edge, ...]]:
    shifted = tuple(
        (source + left_vertices, target + left_vertices)
        for source, target in right_edges
    )
    return left_vertices + right_vertices, tuple(left_edges) + shifted


def verify_endpoint_multiplicativity() -> int:
...
def verify_finite_phase_weights() -> int:
    theta = Fraction(1, 2)
    r1 = Fraction(1, 3)
    r2 = Fraction(2, 3)
    lam = Fraction(2, 5)
    ...
    # The support polynomial vanishes exactly at the two phase values.
    for value in (r1, r2):
...
def verify_richter_quadrature_example() -> int:
    # The uniform law on [0,1] and its two-node Gauss rule agree through
    # degree three, while one law is diffuse and the other is atomic.
    x = sp.symbols("x")
    nodes = (
        sp.Rational(1, 2) - sp.sqrt(3) / 6,
        sp.Rational(1, 2) + sp.sqrt(3) / 6,
    )
...
def verify_adaptive_transition() -> int:
    ...
    phase_up = tuple(
        tuple(value / mu_n[a] for value in successors[a]) for a in range(2)
    )
    ...
    mixture_successors = tuple(
        sum(lam[a] * successors[a][h] for a in range(2)) for h in range(2)
    )
...
def verify_calibrated_multiwell() -> int:
    ...
    phase_laws = tuple(
        tuple(
            dimensions[k] * raw_phase[a][k] / partition[a]
            for k in range(3)
        )
        for a in range(2)
    )
    ...
    expected = tuple(
        sum(lam[a] * phase_laws[a][k] for a in range(2)) for k in range(3)
    )
    ...
    mixed_lower = sum(
        endpoint_law[k] * lower_statistic[k] for k in range(3)
    )
    phase_lower = tuple(
