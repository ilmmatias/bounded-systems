...
So the data itself—not an assumption—will decide between the Gaussian and anomalous regimes.
...
So the infinite-volume positive eigenprofile is not assumed. It is obtained from the asymptotic ratio limit of finite route-count vectors.
...
So we now have the first genuinely computable bridge:
...
And, crucially, every quantity in that chain can be computed by dynamic programming and sparse profile hashing—without diagonalizing the nilpotent adjacency matrix and without assuming the continuum regime in advance.
