...
So cylinder observables are not merely convenient features. They form a dense canonical core for the continuum generator.
...
\frac1{T_p}
\operatorname{diag}(\ell_j),
A,
\operatorname{diag}(r_{p-j-1})
}
]
...
\sum_{v,w}
\overline{\mathsf F}*p(v,w)
\Delta X*{vw}\Delta X_{vw}^\mathsf T.
}
]
...
\operatorname{diag}(d_{\mathrm{out}}),
\qquad
D_{\mathrm{in}}
===============

\operatorname{diag}(d_{\mathrm{in}}).
]
...
That residual global rescaling is not a defect. It is precisely what one expects when no external clock has been postulated.
...
The coefficients above are defined **per graph**. That is important.
...
For each canonical DAG (G), each path horizon (p\le\operatorname{height}(G)), and selected refinement depths (r), compute:
