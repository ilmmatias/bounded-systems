...
That does not assert that each edge requires one unit of physical time or that the system evolves in discrete steps. It records only how many primitive channels were composed.
...
This is the **free** path category: it introduces no equations identifying two different edge sequences. That is not an additional physical axiom. It is the maximally noncommittal construction generated by the existing nodes and primitive channels.

If two paths were declared equal for some further reason, that equality would be extra structure not contained in A1–A5.
...
[
\boxed{\text{Any non-null edge structure is intrinsically noncommutative.}}
]
...
This does not conflict with A1. Algebraic unitality and system closure are different notions.
...
[
\boxed{J\text{ is locally nilpotent}.}
]
...
This is stronger than generic noncommutativity. It is **oriented noncommutativity**:

[
\text{one ordering may exist while its reversal is forbidden.}
]

That relation comes directly from A4 and A5.
...
A corresponding right-creation construction measures incoming channels.

This is not an additional assumption. It is a conditional theorem:

[
\text{given the channel structure, this aggregate exists iff this bound holds.}
]
...
This is precisely why increasing finite node counts were not the correct main approach. The infinite object is not determined by the spectra of its finite truncations.

# 11. The correct primary invariant

The most faithful candidate is not one adjacency matrix and not one spectrum.

It is the marked nonselfadjoint operator algebra
